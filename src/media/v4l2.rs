//! Cameras through `bevy_v4l2`. Behind the `v4l2` feature, Linux only.
//!
...
//! [`MediaSettings::camera`](super::MediaSettings). An app that already runs bevy_v4l2's own
//! `Webcam` entity can share its capture instead: `VideoInput::new(feed.capture.color_tap())`.

use std::{sync::Arc, time::Duration};

use bevy_v4l2::{
    ColorTap,
    capture::{Capture, CaptureConfig, FOURCC_MJPG},
    select::{CameraFormat, FrameFormat, RequestedFormat, choose},
};
...
pub fn cameras() -> Vec<CameraDevice> {
    bevy_v4l2::controls::devices()
        .into_iter()
        .enumerate()
        .map(|(i, info)| {
            let detail = bevy_v4l2::capture::list_modes(&info.path)
                .ok()
...
pub struct Camera {
    ...
    _decoder: Option<bevy_v4l2::mjpeg::Decoder>,
...
impl Camera {
    ...
    pub fn open(id: Option<&str>, width: u32, height: u32, fps: f32) -> Result<Self, String> {
        let path = match id {
            ...
            None => bevy_v4l2::controls::devices()
                .into_iter()
                .next()
                .map(|d| d.path)
                .ok_or("no camera")?,
        };
        let modes = bevy_v4l2::capture::list_modes(&path)
        ...
        let decoder = (!raw).then(|| {
            bevy_v4l2::mjpeg::Decoder::start(capture.clone(), layout.width, layout.height)
        });
        Ok(Self {
            capture,
            tap,
...
fn yuv_to_frame(frame: bevy_v4l2::capture::YuvFrame) -> VideoFrame {
    VideoFrame {
        width: frame.width,
        height: frame.height,
        pixels: Pixels::I420 {
...
/// A `bevy_v4l2` colour tap as a source, for sharing a `Webcam` entity's capture.
